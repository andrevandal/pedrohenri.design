---
title: Estúdio Yoná Matias - Identidade Visual
slug: idv-estudio-yona-matias
description: O Estúdio Yoná Matias é um espaço dedicado a eternizar memórias
  através de fotografias leves, espontâneas e cheias de significado. Cada
  detalhe, cada olhar e cada história são capturados com sensibilidade e
  verdade.
socialImage: https://ik.imagekit.io/pedrohenri/tr:w-1200,h-630/posts/idv_yona-matias/social-image.png
cover:
  provider: imagekit
  image: posts/idv_yona-matias/cover.png
  alternativeText: ""
  source: " "
draft: false
private: false
categories:
  - identidade-visual
updatedAt: 2025-04-14T17:12:00.000Z
createdAt: 2025-04-14T17:12:00.000Z
---
:image{ src="posts/idv_yona-matias/01_cover.png" width="375" height="210" sizes="375px sm:496px lg:960px" alt="" rounded-t rounded-b }

O Estúdio Yoná Matias inicia um novo capítulo da sua história.

Uma identidade visual que nasce do olhar sensível e acolhedor de quem entende que cada fotografia carrega luz, essência e memórias eternas.

Mais do que um símbolo, um manifesto visual sobre capturar o brilho das pessoas, registrar momentos reais e transformar instantes em histórias que ficam.

Naturalidade. Pureza. Harmonia. Acolhimento.

Cada detalhe deste projeto foi pensado para refletir o que o Estúdio é e sempre será: um espaço de conexão verdadeira, de presença e de eternização da vida.

É tudo sobre luz.
É tudo sobre pessoas.
É tudo sobre memórias que duram além do instante.

:image{src="posts/idv_yona-matias/02_simbolo.gif" width="375" height="210" sizes="375px sm:496px lg:960px" alt="" }
:image{src="posts/idv_yona-matias/03_logotipo.gif" width="375" height="210" sizes="375px sm:496px lg:960px" alt="" }
:image{src="posts/idv_yona-matias/04_tipografia.png" width="375" height="210" sizes="375px sm:496px lg:960px" alt="" rounded-t }
:image{src="posts/idv_yona-matias/05_cores.png" width="375" height="105" sizes="375px sm:496px lg:960px" alt="" }
:image{src="posts/idv_yona-matias/06_cartao-de-visitas.png" width="375" height="210" sizes="375px sm:496px lg:960px" alt="" }
:image{src="posts/idv_yona-matias/07_poster.png" width="375" height="210" sizes="375px sm:496px lg:960px" alt="" }
:image{src="posts/idv_yona-matias/08_escritorio.png" width="375" height="210" sizes="375px sm:496px lg:960px" alt="" }
:image{src="posts/idv_yona-matias/09_caixa.png" width="375" height="210" sizes="375px sm:496px lg:960px" alt="" }
:image{src="posts/idv_yona-matias/10_garrafa.png" width="375" height="210" sizes="375px sm:496px lg:960px" alt="" }
:image{src="posts/idv_yona-matias/11_catalogo" width="375" height="210" sizes="375px sm:496px lg:960px" alt="" }
:image{src="posts/idv_yona-matias/12_bordado.png" width="375" height="210" sizes="375px sm:496px lg:960px" alt="" }
:image{src="posts/idv_yona-matias/13_outdoor.png" width="375" height="210" sizes="375px sm:496px lg:960px" alt="" rounded-b }
